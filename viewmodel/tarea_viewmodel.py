...
class TareaViewModel:
    ...
    def agregar_tarea(self, username: str, nombre: str, importancia: int, categoria: int, tiempo: int) -> tuple[bool, str]:
        ...
        if not self.validar_tiempo(tiempo):
            return False, "El tiempo debe ser igual o mayor a 0"
    ...
    def actualizar_nombre_tarea(self, username: str, nombre_antiguo: str, nombre_nuevo: str) -> tuple[bool, str]:
    ...
    def actualizar_importancia(self, username: str, nombre_tarea: str, importancia: int) -> tuple[bool, str]:
        #actualiza la importancia de una tarea. Retorna (éxito, mensaje)
        if not self.validar_importancia(importancia):
            return False, "Importancia debe ser 1, 2 o 3"
        
        if self.repository.actualizar_campo_tarea(username, nombre_tarea, "importancia de la Tarea", importancia):
            return True, f"Importancia de '{nombre_tarea}' actualizada"
        return False, "Error al actualizar importancia"
    
    def actualizar_categoria(self, username: str, nombre_tarea: str, categoria: int) -> tuple[bool, str]:
        #actualiza la categoría de una tarea. Retorna (éxito, mensaje)
        if not self.validar_categoria(categoria):
            return False, "Categoría debe ser 1, 2 o 3"
        
        if self.repository.actualizar_campo_tarea(username, nombre_tarea, "Categoria", categoria):
            return True, f"Categoría de '{nombre_tarea}' actualizada"
        return False, "Error al actualizar categoría"
    
    def actualizar_tiempo(self, username: str, nombre_tarea: str, tiempo: int) -> tuple[bool, str]:
        #actualiza el tiempo de una tarea. Retorna (éxito, mensaje)
        if not self.validar_tiempo(tiempo):
            return False, "El tiempo debe ser mayor a 0"
        
        if self.repository.actualizar_campo_tarea(username, nombre_tarea, "Tiempo", tiempo):
