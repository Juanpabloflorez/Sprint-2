...
class ConsoleView:
    ...
    def crear_usuario(self):
        ...
        ConsoleView.mostrarMenuUsuario(self,username)
        return exito
    ...
    def agregar_tarea(self,username):
    ...
    def actualizar_tarea(self,username):
    ...
    def eliminar_tarea(self,username):
        #eliminar una tarea
    ...
    def marcar_completada(self,username):
    ...
    def menu_principal(self):
        #Menú inicial para el inicio
        while True:
            self.mostrar_mensaje("\n=== BIENVENIDO AL ORGANIZADOR DE TAREAS ===")
            self.mostrar_mensaje("1. Crear usuario")
            self.mostrar_mensaje("2. Iniciar sesión")
            self.mostrar_mensaje("3. Salir")
            ...
            if opcion == 1:
                ...
                self.mostrar_mensaje("*Finalizando sesion*")
                break
            else:
                opcion = int(input(self.mostrar_mensaje("Opción no válida")))

    def mostrarMenuUsuario(self,username):
        #Menú para una cuenta determinada
        while True:
            self.mostrar_mensaje("1. Agregar tarea")
            self.mostrar_mensaje("2. Actualizar tarea")
            self.mostrar_mensaje("3. Eliminar tarea")
            self.mostrar_mensaje("4. Ver tareas")
            self.mostrar_mensaje("5. Marcar tarea como completada")
            self.mostrar_mensaje("6. Salir")

            opcion = self.solicitar_input_int("\nSeleccione una opción: ")
            
            if opcion == 1:
                self.agregar_tarea(username)
            elif opcion == 2:
                self.actualizar_tarea(username)
            elif opcion == 3:
                self.eliminar_tarea(username)
            elif opcion == 4:
                self.leer_tareas(username)
            elif opcion == 5:
                self.marcar_completada(username)
            elif opcion == 6:
                self.mostrar_mensaje("*Finalizando sesion*")
                break
            else:
